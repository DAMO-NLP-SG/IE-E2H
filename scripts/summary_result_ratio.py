...
class ResultSummary:
    ...
    def get_valid_folder(self, model_folder, file_map):
        ...
        sub_folder_list = []
        for parent_folder in os.listdir(model_folder):
            parent_folder = os.path.join(model_folder, parent_folder)
            if os.path.isdir(parent_folder):
                for child_folder in os.listdir(parent_folder):
                    sub_folder_list.append(os.path.join(parent_folder, child_folder))

        sub_folder_list = sorted(sub_folder_list)

        for sub_folder in sub_folder_list:
            if sub_folder.endswith('log') or sub_folder.endswith('err'):
                continue
...
def main():
    ...
    parser.add_argument('-model_name', dest='model_name', default='_',
                        help='Model Name')
    parser.add_argument('-dataset_name', dest='dataset_name', default='_',
                        help='Dataset Name')
    parser.add_argument('-strategy', dest='strategy', default='_',
                        help='eth or default.')
