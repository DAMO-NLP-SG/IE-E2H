...
from uie.seq2seq.data_collator.meta_data_collator_skill_event import (
...
def decomposition(dataset, data_args):
...
def build_easy(datasets, data_args):
    # logger.info(datasets["train"])
    datasets["train"] = decomposition(datasets["train"], data_args)
    datasets["validation"] = decomposition(datasets["validation"], data_args)
    datasets["test"] = decomposition(datasets["test"], data_args)


def process_datasets(datasets, data_args):
    if data_args.stage == "easy":
        build_easy(datasets, data_args)
    else:
...
def main():
    ...
    process_datasets(datasets, data_args)
    ...
    if training_args.do_train and "uie" in model_args.model_name_or_path:
        ...
        for special_token in [constants.type_start, constants.type_end, constants.text_start, constants.span_start,
                              constants.spot_prompt, constants.asoc_prompt]:
            if special_token not in tokenizer.get_vocab():
                to_add_special_token += [special_token]
        tokenizer.add_special_tokens(
            {"additional_special_tokens": tokenizer.special_tokens_map_extended[
