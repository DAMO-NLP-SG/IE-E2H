...
for index in $(seq 1 ${run_time}); do
  main_output_dir=${model_folder}
  easy_output_dir=${easy_model_folder}
  hard_output_dir=${hard_model_folder}
  ...
  ratio_data_folder=${data_folder}/ratio/seed${index}
  echo "ratio_data_folder: " ${ratio_data_folder}

  for ratio in $(ls ${ratio_data_folder})
  do

    run_data_folder=${ratio_data_folder}/${ratio}
    echo "run_data_folder: " ${run_data_folder}
    main_run_output_folder=${main_output_dir}/${ratio}_run${index}
    echo "main_run_output_folder: " ${main_run_output_folder}
    easy_run_output_folder=${easy_output_dir}/${ratio}_run${index}
    echo "easy_run_output_folder: " ${easy_run_output_folder}
    hard_run_output_folder=${hard_output_dir}/${ratio}_run${index}
    echo "hard_run_output_folder: " ${hard_run_output_folder}

    if [[ ! -d ${main_run_output_folder} ]]
    then
      mkdir ${main_run_output_folder}
    else
      continue
    fi

    if [[ ! -d ${easy_run_output_folder} ]]
    then
      echo "Easy Stage ..."
      mkdir ${easy_run_output_folder}
      eval_steps=$(python scripts/get_eval_batch_num.py ${run_data_folder}/train.json ${batch_size} 10)
      ...
      CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES} ${run_command} skill_${task}.py \
        ...
        --output_dir=${easy_run_output_folder} \
    ...
    if [[ ! -d ${hard_run_output_folder} ]]
    then
      echo "Hard Stage ..."
      mkdir ${hard_run_output_folder}
      CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES} ${run_command} skill_${task}.py \
        --do_train --do_eval --do_predict ${constraint_decoding} ${fp16} \
        --stage hard \
        --report_to wandb \
        ...
        --gradient_accumulation_steps=${gradient_accumulation_steps} \
        --per_device_eval_batch_size=$((batch_size * 4)) \
        --output_dir=${hard_run_output_folder} \
        --overwrite_output_dir \
        --model_name_or_path=${easy_run_output_folder} \
        --learning_rate=${lr} \
        --source_prefix="${task_name}: " \
        --lr_scheduler_type=${lr_scheduler} \
        --label_smoothing_factor=${label_smoothing} \
    ...
    CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES} ${run_command} skill_${task}.py \
        ...
        --gradient_accumulation_steps=${gradient_accumulation_steps} \
        --per_device_eval_batch_size=$((batch_size * 4)) \
        --output_dir=${main_run_output_folder} \
        --overwrite_output_dir \
        --model_name_or_path=${hard_run_output_folder} \
        --learning_rate=${lr} \
        --source_prefix="${task_name}: " \
        --lr_scheduler_type=${lr_scheduler} \
        --label_smoothing_factor=${label_smoothing} \
